...
for name in `cat names.list ` ; do pwd | awk -F 'CYS' '{print $NF}' | awk -v "var=${name}" -F '/' '{print "cat /home/danielza/Work/Current/Covalentizer/databases/PDB_Final/Ligands/Prepared/" $2 "/boronic_index.smi | awk '\''$2==\""var"\"'\'' | awk '\''{print $1}'\''"}' ; done > tmp.sh ; chmod 777 tmp.sh ; ./tmp.sh > smiles.list
rm tmp.sh
